"use client"
import ChapterBanner from "../shared/chapter-banner";
import Button from "../ui/button";
import WorkCard from "../ui/work-card";
import { AnimatedText } from "../animations/anim";
import { useMediaQuery } from "@/hooks/useMediaQuery";

const Work = () => {
  const isMobile = useMediaQuery('(max-width: 767px)');
  const isTabletToLg = useMediaQuery('(min-width: 768px) and (max-width: 1279px)');
  const isXlAndUp = useMediaQuery('(min-width: 1280px)');

  const workItems = [
    {
      date: "01/03",
      title: "Laura Hartmann Films",
      category: "Videographer site",
      image: "/work-laura.webp",
      description: "Your love story captured authen...",
      href: "https://laurahartmannfilms.com/",
    },
    {
      date: "02/03",
      title: "Moonrise Photos",
      category: "Photographer site",
      image: "/work-katie.webp",
      description: "Nostalgic & Sincere images for non-...",
      href: "https://moonrise.photos/",
    },
    {
      date: "03/03",
      title: "Piks By Vik",
      category: "Photographer site",
      image: "/work-victoria.webp",
      description: "Capturing Captivating & Genuine...",
      href: "https://www.piksbyvikphotography.com",
    },
  ];

  const mobileOrderItems = [workItems[2], workItems[0], workItems[1]];

  const renderMobileLayout = () => (
    <div className="space-y-8">
      <div className="mb-8">
        <AnimatedText
          text="Every project tells a story — of vision brought to life, of brands finding their voice, and of design that works harder than it looks. Here's a glimpse at what we've built for those who dared to stand out."
          className="text-section-heading-small font-noto-serif font-semibold text-primary-dark italic leading-tight max-w-4xl"
        />
      </div>
      {mobileOrderItems.map((item) => (
        <WorkCard key={item.date} {...item} />
      ))}
    </div>
  );

  const renderTabletLayout = () => (
    <div>
      <div className="mb-12">
        <AnimatedText
          text="Every project tells a story — of vision brought to life, of brands finding their voice, and of design that works harder than it looks. Here's a glimpse at what we've built for those who dared to stand out."
          className="text-section-heading-small font-noto-serif font-semibold text-primary-dark italic leading-tight max-w-4xl"
        />
      </div>
      <div className="grid grid-cols-2 gap-8 lg:gap-12">
        {workItems.map((item, index) => (
          <div key={index} className={index === 2 ? "col-span-2 max-w-md mx-auto" : ""}>
            <WorkCard {...item} />
          </div>
        ))}
      </div>
    </div>
  );

  const renderDesktopLayout = () => (
    <div className="relative">
      {/* First row: Text + Right card */}
      <div className="flex items-start justify-between mb-6 lg:mb-8">
        {/* Section intro - left side */}
        <div className="flex-1 xl:max-w-3xl 2xl:max-w-5xl pr-8">
          <AnimatedText
            text="Every project tells a story — of vision brought to life, of brands finding their voice, and of design that works harder than it looks. Here's a glimpse at what we've built for those who dared to stand out."
            className="text-section-heading-small font-noto-serif font-semibold text-primary-dark italic leading-tight"
          />
        </div>

        {/* Third card - right side, aligned with text top */}
        <div className="flex-shrink-0 scale-110 origin-top-right">
          <WorkCard
            date={workItems[2].date}
            title={workItems[2].title}
            category={workItems[2].category}
            image={workItems[2].image}
            description={workItems[2].description}
            href={workItems[2].href}
          />
        </div>
      </div>

      {/* Second row: Two cards - positioned absolutely with equal spacing */}
      <div className="absolute top-40 left-0 right-0">
        <div className="relative w-full">
          {/* First card - left position */}
          <div className="absolute left-0 origin-top-left">
            <WorkCard
              date={workItems[0].date}
              title={workItems[0].title}
              category={workItems[0].category}
              image={workItems[0].image}
              description={workItems[0].description}
              href={workItems[0].href}
            />
          </div>

          {/* Second card - center position with equal spacing */}
          <div className="absolute left-1/2 transform -translate-x-1/2 mt-6 lg:mt-40 origin-top-left scale-90">
            <WorkCard
              date={workItems[1].date}
              title={workItems[1].title}
              category={workItems[1].category}
              image={workItems[1].image}
              description={workItems[1].description}
              href={workItems[1].href}
            />
          </div>
        </div>
      </div>
    </div>
  );

  return (
    <section id="work" className="w-full bg-primary-light">
      <ChapterBanner
        chapterNumber="Chapter 3"
        chapterTitle="OUR RECENT WORK"
        className="border-b border-primary-dark"
      />

      <div className="px-4 md:px-[2.5vw] py-12 md:py-16">
        {isMobile && renderMobileLayout()}
        {isTabletToLg && renderTabletLayout()}
        {isXlAndUp && renderDesktopLayout()}

        {/* Get Added button */}
        <div className="mt-12 xl:mt-54">
          <Button
            textColor="#FFFDE4"
            hoverTextColor="#FFFDE4"
            borderColor="#888888"
            className="font-aileron font-semibold text-body py-4 bg-accent-primary"
            showArrow={true}
            href="#book-call"
          >
            Get added here
          </Button>
        </div>
      </div>
    </section>
  );
};

export default Work;
